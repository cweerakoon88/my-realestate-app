  ...
  return (
    <div style={c.card}>
      <div style={c.cardTop}>
        <div style={c.cardLeft}>
          ...
          <h3 style={c.cardTitle}>
            {req.bedrooms ? `${req.bedrooms}-bed ` : ''}{req.property_type || 'Property'} in {req.location}
          </h3>
          <div style={c.cardMeta}>
            ...
            {req.land_size_min && <span style={c.metaItem}>📐 {req.land_size_min.toLocaleString()}m²+</span>}
          </div>
        </div>
      ...
      {req.proximity_preferences?.length > 0 && (
        <div style={c.chips}>
          {req.proximity_preferences.map(p => (
            <span key={p} style={c.chip}>{proximityLabel(p)}</span>
          ))}
        </div>
      )}
      ...
      {expanded && (
        <div style={c.expanded}>
          ...
          {req.notes && (
            <div style={c.expandRow}>
              <span style={c.expandLabel}>Notes</span>
              <span style={c.expandVal}>{req.notes}</span>
            </div>
          )}
          <a href={`mailto:${req.buyer_email}?subject=Property offer for your requirement in ${req.location}`} style={c.contactBtn}>
            ✉ Contact this buyer
          </a>
        </div>
      )}

      <button style={c.expandBtn} onClick={() => setExpanded(e => !e)}>
...
function ListingCard({ listing }) {
  ...
  return (
    <div style={c.card}>
      ...
      {expanded && (
        <div style={c.expanded}>
          <p style={{ fontSize: '14px', color: '#555', fontFamily: 'system-ui,sans-serif', lineHeight: 1.6, marginBottom: '1rem' }}>
            {listing.description}
          </p>
          <div style={c.expandRow}>
            <span style={c.expandLabel}>Listed by</span>
            <span style={c.expandVal}>{listing.seller_name}</span>
          </div>
          <a href={`mailto:${listing.seller_email}?subject=Enquiry about ${listing.title}`} style={{ ...c.contactBtn, background: '#1a6fa8' }}>
            ✉ Contact seller
          </a>
        </div>
      )}
...
export default function Marketplace() {
  ...
  return (
    <div style={p.page}>
      {/* NAV */}
      <nav style={p.nav}>
        <a href="/" style={p.navLogo}>Prop<span style={{ color: '#b8924a' }}>Match</span></a>
        <div style={p.navLinks}>
          <Link href="/services" style={p.navLink}>Services</Link>
          <Link href="/post" style={p.navLink}>Post Requirement</Link>
        </div>
      </nav>
      ...
      <div style={p.container}>
        ...
        {/* PAGE HEADER */}
        <div style={p.header}>
          <a href="/" style={p.backBtn}>← Back to home</a>
        ...
        {/* TABS */}
        <div style={p.tabBar}>
          <button style={tab === 'buyers' ? p.tabActive : p.tab} onClick={() => setTab('buyers')}>
            🏠 Buyers <span style={p.tabCount}>{requirements.length}</span>
          </button>
          <button style={tab === 'sellers' ? p.tabActive : p.tab} onClick={() => setTab('sellers')}>
            🏷️ Sellers <span style={p.tabCount}>{listings.length}</span>
          </button>
        </div>
        ...
        <div style={p.actionRow}>
          <div style={p.filters}>
            <input style={p.filterInput} placeholder="🔍 Filter by suburb..." value={filterLocation}
            ...
            {(filterLocation || filterType || filterBeds) && (
              <button style={p.clearBtn} onClick={() => { setFilterLocation(''); setFilterType(''); setFilterBeds('') }}>
                Clear
              </button>
            )}
          </div>
          {tab === 'buyers' && (
            <Link href="/post" style={p.actionBtn}>+ Post requirement</Link>
          )}
          {tab === 'sellers' && (
            <button style={p.actionBtn} onClick={() => { setShowSellerForm(true); setSellerSuccess(false) }}>
              + List a property
            </button>
          )}
        </div>
        ...
        {/* CONTENT */}
        {loading ? (
          <div style={p.loading}>Loading listings...</div>
        ) : (
          <>
            {tab === 'buyers' && (
              <>
                {filteredReqs.length === 0 ? (
                  <div style={p.empty}>
                    ...
                    <p style={p.emptyText}>No buyer requirements found.</p>
                    <Link href="/post" style={p.actionBtn}>Be the first to post</Link>
                  </div>
                ) : (
                  <div style={p.grid}>
                    {filteredReqs.map(r => <BuyerCard key={r.id} req={r} />)}
                  </div>
                )}
              </>
            )}
              ...
              <>
                {filteredListings.length === 0 ? (
                  <div style={p.empty}>
                    ...
                    <p style={p.emptyText}>No properties listed yet.</p>
                    <button style={p.actionBtn} onClick={() => setShowSellerForm(true)}>List the first property</button>
                  </div>
                ) : (
                  <div style={p.grid}>
      ...
      {showSellerForm && !sellerSuccess && (
        <SellerPostForm
          ...
          onSuccess={() => { setSellerSuccess(true); fetchAll() }}
        />
      )}
      ...
      {sellerSuccess && (
        <div style={fs.overlay}>
          <div style={{ ...fs.modal, maxWidth: '420px', textAlign: 'center', padding: '2.5rem 2rem' }}>
            ...
            <p style={{ fontSize: '14px', color: '#666', fontFamily: 'system-ui,sans-serif', lineHeight: 1.6, marginBottom: '1.5rem' }}>
              Your property is now visible to buyers. They can contact you directly.
            </p>
            <div style={fs.teamNote}>
              <span>👋</span>
              <p style={{ fontSize: '13px', color: '#555', fontFamily: 'system-ui,sans-serif', margin: 0, lineHeight: 1.5 }}>
                <strong>Melina & Mikayla</strong> will be in touch shortly to help with next steps.
              </p>
            </div>
            <button style={fs.submitBtn} onClick={() => { setSellerSuccess(false); setShowSellerForm(false) }}>
              Back to marketplace
            </button>
          </div>
        </div>
...
const p = {
  ...
  nav: { position: 'sticky', top: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 2rem', background: 'rgba(250,248,243,0.95)', backdropFilter: 'blur(10px)', borderBottom: '1px solid #e8e0d0' },
  navLogo: { fontFamily: 'Georgia,serif', fontSize: '1.3rem', fontWeight: '600', color: '#1a1714', textDecoration: 'none' },
  navLinks: { display: 'flex', alignItems: 'center', gap: '1.5rem' },
  navLink: { fontFamily: 'system-ui,sans-serif', fontSize: '13px', color: '#4a4540', textDecoration: 'none' },
  container: { maxWidth: '900px', margin: '0 auto', padding: '2rem 1.5rem' },
  header: { marginBottom: '2rem' },
  backBtn: { display: 'inline-block', fontSize: '13px', fontFamily: 'system-ui,sans-serif', color: '#9c7c4a', textDecoration: 'none', marginBottom: '1rem' },
...
const c = {
  card: { background: '#fff', border: '1px solid #e8e0d0', borderRadius: '12px', padding: '1.25rem 1.5rem', transition: 'box-shadow 0.2s' },
  ...
  expandLabel: { color: '#bbb', minWidth: '60px', flexShrink: 0 },
  expandVal: { color: '#555' },
